...
date: "09/03/2020 updated `r format(Sys.time(), '%B %d, %Y')`"
output: 
  html_document:
    toc: true
    toc_float: true
...
# Data
preston_crime <- readRDS("data/pcrime-spatstat.rds") # Loading data
...
plotcv(bw_choice); abline(v = bw_choice$hcv, # Hcv stands for bandwidth selected by the cross-validated log-likelihood function.
                          lty = 2, col = "red")

# Print the best bandwidth
print(bw_choice$hcv)
h <- bw_choice$hcv # Selecting bandwith in a column
```
...
ncol <- length(seg$gridx) # Selecting one of the coordinates to se length
...
p_value <- list(x = seg$gridx,
                y = seg$gridy,
                z = matrix(seg$p[, "Violent crime"]< 0.05, # Selecting significant violent crime probabilities from the column p for violent crimes only
                                ncol = ncol))
image(p_value)
...
segmap(prob_violent, p_value, 0.05, 0.15) # I am setting 0.05 as the lower level value and 0.15 as the high value.
```
